...
{
    public partial class ReportSms1 :Base
    {
        DataTable dt;
        ...
        protected void BindGrid()
        {
            ...
            try
            {
                ...
                {
                    dtUser = srch.SrchReportSmsDtls(ClientId, 0, ddlUserName.SelectedValue.ToString(), txtSrchNo.Text.Trim(), ddlDirection.SelectedValue.ToString(), txtFrmDate.Text.Trim(), txtToDate.Text.Trim(), StartTime, EndTime);
                }
                else if (RoleId == 3)
                {
                    dtUser = srch.SrchReportSmsDtls(ClientId, 0, ddlUserName.SelectedValue.ToString(), txtSrchNo.Text.Trim(), ddlDirection.SelectedValue.ToString(), txtFrmDate.Text.Trim(), txtToDate.Text.Trim(), StartTime, EndTime, DeptId);
                }
                else
                {
                    dtUser = srch.SrchReportSmsDtls(ClientId, UserId, ddlUserName.SelectedValue.ToString(), txtSrchNo.Text.Trim(), ddlDirection.SelectedValue.ToString(), txtFrmDate.Text.Trim(), txtToDate.Text.Trim(), StartTime, EndTime);
                }
        ...
        protected void btnsavetopdf_Click(object sender, EventArgs e)
        {
            ...
            using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream())
            {
                ...
                cell0 = PhraseCell(new Phrase(" Message Log", FontFactory.GetFont("Arial", 16, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)), PdfPCell.ALIGN_CENTER);
                cell0.Colspan = 8;
                cell0.BorderWidth = 0;
                table.AddCell(cell0);
                ...
                Response.AddHeader("Content-Disposition", "attachment; filename=SmsReport.pdf");
                Response.ContentType = "application/pdf";
        ...
        public void GrideTable()
        {
            ...
            if (dtpdf.Rows.Count > 0)
            {
                ...
                Message = Message + ("<td align='center'>");
                Message = Message + ("<b>SMS From/To</b>");
                Message = Message + ("</td>");
                ...
                Message = Message + ("<td align='center'>");
                Message = Message + ("<b>Message Text</b>");
                Message = Message + ("</td>");
                ...
                for (int i = 0; i < dtpdf.Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < 8; j++)
                    {
                        ...
                        cellText = dtpdf.Rows[i][j].ToString();
                        //cellText = cellText.Replace("http://bit.ly/WAY22", "").Replace("http://bit.ly/1MQjp1Z", "");
                        if (j == 5)
                        {
                            //cellText = HttpContext.Current.Server.UrlEncode(dtpdf.Rows[i][j].ToString());
                            cellText = HttpContext.Current.Server.UrlPathEncode(dtpdf.Rows[i][j].ToString());
                            cellText = cellText.Replace("%20", " ").Replace("%0a%20", " ").Replace("%0a", " ");
                        }
                        if (j == 7)
                        {
                            if (cellText == "1")
        ...
        protected void Send_Click(object sender, EventArgs e)
        {
            ...
            if (RbtnYou.Checked || RbtnOther.Checked)
            {
                ...
                msg.Append("<b>The below table has the details for user Message Log.</b>");
                msg.AppendLine(); msg.AppendLine();
        ...
        private void MailsendYou(string Msgbody)
        {
            try
            {
                ...
                send.SendEmail(EmailId, "Message Log at " + GetCurrentDateTimeByUserId(), Msgbody, ClientId);
        ...
        private void MailsendOther(string Msgbody)
        {
            ...
            if (txtMailTo.Text != "" && Regex.IsMatch(txtMailTo.Text.Trim(), emailpattern))
            {
                try
                {
                    ...
                    dt = new DataTable();
                    send = new SendMailBAL();
                    send.SendEmail(txtMailTo.Text, "Message Log at " + GetCurrentDateTimeByUserId(), Msgbody, ClientId);
                    lblMsg.Text = "Mail sent successfully";
                    lblMsg.ForeColor = System.Drawing.Color.Green;
                }
                catch (Exception)
        ...
        private void LoadChartData(DataTable initialDataSource)
        {
            for (int i = 2; i < initialDataSource.Columns.Count; i++)
            {
                ...
                foreach (DataRow dr in initialDataSource.Rows)
                {
                    ...
                    series.Points.AddXY(string.IsNullOrEmpty(dr["date2"].ToString()) ? Convert.ToDateTime(dr["date1"]).ToString("dd MMM yyyy") : Convert.ToDateTime(dr["date2"]).ToString("dd MMM yyyy"), y);
        ...
        protected DataTable GetData()
        {
            ...
            try
            {
                ...
                if (RoleId == 1 || RoleId == 2)
                {
                    dt1 = srch.SrchReportSmsDtlsForChart(ClientId, 0, ddlUserName.SelectedValue.ToString(), txtSrchNo.Text.Trim(), ddlDirection.SelectedValue.ToString(), txtFrmDate.Text.Trim(), txtToDate.Text.Trim(), StartTime, EndTime);
                }
                else if (RoleId == 3)
                {
                    dt1 = srch.SrchReportSmsDtlsForChart(ClientId, 0, ddlUserName.SelectedValue.ToString(), txtSrchNo.Text.Trim(), ddlDirection.SelectedValue.ToString(), txtFrmDate.Text.Trim(), txtToDate.Text.Trim(), StartTime, EndTime, DeptId);
                }
                else
                {
                    dt1 = srch.SrchReportSmsDtlsForChart(ClientId, UserId, ddlUserName.SelectedValue.ToString(), txtSrchNo.Text.Trim(), ddlDirection.SelectedValue.ToString(), txtFrmDate.Text.Trim(), txtToDate.Text.Trim(), StartTime, EndTime);
                }
            }
            catch (Exception)
            {
