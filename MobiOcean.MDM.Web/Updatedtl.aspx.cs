...
{
    public partial class Updatedtl : Base
    {
        DDLBAL ddlbal;
        RamuSearch rsr;
        DataTable dt;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            lblMsg.Text = string.Empty;// Response.Redirect("UserDashBoard.aspx");

            if (!IsPostBack)
            {
                BindUsrName(ddlUserName);
                BindGrid();
            }
        }
        ...
        protected void BindUsrName(DropDownList ddlUserName)
        {
            try
            {
                ddlbal = new DDLBAL();
        ...
        protected void BindGrid()
        {
            ...
            try
            {
                rsr = new RamuSearch();
                dt = new DataTable();
                //dt = rsr.GetAck(ClientId, ddlUserName.SelectedValue.ToString());
                if (RoleId == 1 || RoleId == 2)
                {
                    dtUser = rsr.GetAck(ClientId, ddlUserName.SelectedValue.ToString(), 0, 0);
                }
                else if (RoleId == 3)
                {
                    dtUser = rsr.GetAck(ClientId, ddlUserName.SelectedValue.ToString(), 0, DeptId);
                }
                else
                {
                    dtUser = rsr.GetAck(ClientId, ddlUserName.SelectedValue.ToString(), UserId, 0);
                }
                grdDept.DataSource = dtUser;
        ...
        protected void btnsavetopdf_Click(object sender, EventArgs e)
        {
            ...
            using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream())
            {
                ...
                cell2 = PhraseCell(new Phrase(" Profile Update Detail ", FontFactory.GetFont("Arial", 10, Font.BOLD, BaseColor.BLACK)), PdfPCell.ALIGN_CENTER);
                cell2.Colspan = 8;
                cell2.BorderWidth = 0;
                table.AddCell(cell2);
                cell1 = PhraseCell(new Phrase("Device/UserName : " + ddlUserName.SelectedItem.ToString(), FontFactory.GetFont("Arial", 10, Font.BOLD, BaseColor.BLACK)), PdfPCell.ALIGN_CENTER);
                cell1.Colspan = 8;
                cell1.BorderWidth = 0;
                table.AddCell(cell1);
                int k = 1;
                for (int x = 0; x < grdDept.Columns.Count; x++)
                {
                    if(x==4)
                    {
                        continue;
                    }
                    string cellText = Server.HtmlDecode(grdDept.HeaderRow.Cells[x].Text);
                ...
                {
                    for (int j = 0; j < grdDept.Columns.Count; j++)
                    {
                        if (j >= 4)
                        {
                            continue;
                ...
                Response.AddHeader("Content-Disposition", "attachment; filename=ProfileUpdateDetails.pdf");
                Response.ContentType = "application/pdf";
        ...
        public void GrideTable()
        {
            ...
            if (dtpdf.Rows.Count > 0)
            {
                ...
                Message = Message + ("<b style=' font-size: 20px;'>Profile Updation Report</b>");
                Message = Message + ("</td>");
                Message = Message + ("</tr>");
                Message = Message + ("<tr>");
                Message = Message + ("<td colspan='8' align='center'>");
        ...
        protected void Send_Click(object sender, EventArgs e)
        {
            ...
            if (RbtnYou.Checked || RbtnOther.Checked)
            {
                ...
                msg.Append("<b>The below table has the details for Profile Updation.</b>");
                msg.AppendLine(); msg.AppendLine();
        ...
        private void MailsendYou(string msgbody)
        {
            try
            {
                ...
                send.SendEmail(EmailId, "Profile Update Report at " + GetCurrentDateTimeByUserId(), msgbody, ClientId);
        ...
        private void MailsendOther(string msgbody)
        {
            ...
            if (txtMailTo.Text != "" && Regex.IsMatch(txtMailTo.Text.Trim(), emailpattern))
            {
                try
                {
                    ...
                    dt = new DataTable();
                    send = new SendMailBAL();
                    send.SendEmail(txtMailTo.Text, "Profile Update Report at " + GetCurrentDateTimeByUserId(), msgbody, ClientId);
                    lblMsg.Text = "Mail sent successfully";
                    lblMsg.ForeColor = System.Drawing.Color.Green;
                }
                catch (Exception)
        ...
        protected void lnkbtn_click(object sender, EventArgs e)
        {
            try
            {
                ...
                Label lbl = ((Label)grdDept.Rows[gvr.RowIndex].FindControl("lblUserId"));
                Label lbl1 = ((Label)grdDept.Rows[gvr.RowIndex].FindControl("lblDeptCode"));
                Label lbl2 = ((Label)grdDept.Rows[gvr.RowIndex].FindControl("lblDeptName"));
                rsr = new RamuSearch();
                Gdv.DataSource = rsr.GetMoreUpdationDetail(Convert.ToInt32(lbl.Text));
                Gdv.DataBind();
                MP1.Show();
            }
            catch (Exception)
