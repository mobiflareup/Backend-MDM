...
{
    public partial class TAOthersDtl : Base
    {
        GingerboxSrch gsrch;
        ...
        private void BindGrid()
        {
            ...
            dt = gsrch.GetExtraDetailsbyMasterId(MasterID);
            ViewState["dtUser"] = dt;
            if (dt.Rows.Count > 0)
            {
                taExtra.DataSource = dt;
                taExtra.DataBind();
        ...
        protected void taExtra_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            taExtra.PageIndex = e.NewPageIndex;
            taExtra.EditIndex = -1;
            BindGrid();
        }
        ...
        protected void btnsavetopdf_Click(object sender, EventArgs e)
        {
            ...
            using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream())
            {
                ...
                cell1 = PhraseCell(new Phrase("Extra Expenses Details", FontFactory.GetFont("Arial", 10, Font.BOLD, BaseColor.BLACK)), PdfPCell.ALIGN_CENTER);
                ...
                for (int x = 0; x < taExtra.Columns.Count; x++)
                {
                    if (x == 0 || x > cellCount)
                    {
                        continue;
                    }
                    string cellText = Server.HtmlDecode(taExtra.HeaderRow.Cells[x].Text);
                ...
                {
                    for (int j = 0; j < taExtra.Columns.Count; j++)
                    {
                        if (j > cellCount - 1)
                        {
                            continue;
                        ...
                        if (j == 2)
                        {
                            cellText = Convert.ToDateTime(dtpdf.Rows[i][j]).ToString("dd-MMM-yyyy hh:mm");
                        }
                        if (j == 3)
                        {
        ...
        public void GrideTable()
        {
            ...
            if (dtpdf.Rows.Count > 0)
            {
                ...
                Message = Message + ("<td align='center'>");
                Message = Message + ("<b>Creation Date</b>");
                Message = Message + ("</td>");
        ...
        protected void Send_Click(object sender, EventArgs e)
        {
            ...
            if (RbtnYou.Checked || RbtnOther.Checked)
            {
                ...
                msg.Append("<b>The below table has the details for Extra Expenses claimed by user.</b>");
                msg.AppendLine(); msg.AppendLine();
        ...
        private void MailsendYou(string Msgbody)
        {
            try
            {
                ...
                send.SendEmail(EmailId, "Extra Expenses Details at " + GetCurrentDateTimeByUserId(), Msgbody, ClientId);
        ...
        private void MailsendOther(string Msgbody)
        {
            ...
            if (txtMailTo.Text != "" && Regex.IsMatch(txtMailTo.Text.Trim(), emailpattern))
            {
                try
                {
                    ...
                    dt = new DataTable();
                    send = new SendMailBAL();
                    send.SendEmail(txtMailTo.Text, "Extra Expenses Details at " + GetCurrentDateTimeByUserId(), Msgbody, ClientId);
                    lblMsg.Text = "Mail sent successfully";
                    lblMsg.ForeColor = System.Drawing.Color.Green;
                }
                catch (Exception)
