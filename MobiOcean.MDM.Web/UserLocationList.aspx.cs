...
{
    public partial class UserLocationList : Base
    {
        ...
        protected void BindGrid()
        {
            ...
            try
            {
                ...
                if (RoleId == 1 || RoleId == 2)
                {
                    dtUser = srch.SrchUserLocationDtls(ClientId, ddlUserName.SelectedValue.ToString(), txtDeviceName.Text.Trim(), FrmDateTime, ToDateTime, 0);
                }
                else if (RoleId == 3)
                {
                    dtUser = srch.SrchUserLocationDtls(ClientId, ddlUserName.SelectedValue.ToString(), txtDeviceName.Text.Trim(), FrmDateTime, ToDateTime, 0, DeptId);
                }
                else
                {
                    dtUser = srch.SrchUserLocationDtls(ClientId, ddlUserName.SelectedValue.ToString(), txtDeviceName.Text.Trim(), FrmDateTime, ToDateTime, UserId);
                }
        ...
        protected void btnsavetopdf_Click(object sender, EventArgs e)
        {
            ...
            using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream())
            {
                ...
                {
                    cell1 = PhraseCell(new Phrase(" Location Report     ", FontFactory.GetFont("Arial", 16, Font.BOLD, BaseColor.BLACK)), PdfPCell.ALIGN_CENTER);
                }
                else
                {
                    cell1 = PhraseCell(new Phrase(ddlUserName.SelectedItem.ToString() + " Location Report ", FontFactory.GetFont("Arial", 16, Font.BOLD, BaseColor.BLACK)), PdfPCell.ALIGN_CENTER);
                }
                cell1.Colspan = 8;
                cell1.BorderWidth = 0;
                table.AddCell(cell1);
                ...
                {
                    cell2 = PhraseCell(new Phrase(" Device Name : " + txtDeviceName.Text, FontFactory.GetFont("Arial", 10, Font.BOLD, BaseColor.BLACK)), PdfPCell.ALIGN_CENTER);
                    cell2.Colspan = 8;
                    cell2.BorderWidth = 0;
                    table.AddCell(cell2);
                }
                ...
                Response.AddHeader("Content-Disposition", "attachment; filename=LocationReport.pdf");
                Response.ContentType = "application/pdf";
        ...
        public void GrideTable()
        {
            ...
            if (dtpdf.Rows.Count > 0)
            {
                ...
                try
                {
                    if (Convert.ToInt16(ddlUserName.SelectedValue) == 0)
                    {
                        UName = "User Location Report";
                    }
                    else
                    {
                        UName = UName + "<br/>" + ddlUserName.SelectedItem.ToString() + "Location Report ";
                    }
                    if (txtDeviceName.Text != "")
                    {
                        DevName = DevName + "<br/>" + "  Device : " + txtDeviceName.Text;
                    }
                    if (txtFrmDt.Text != "")
                    {
                ...
                Message = Message + "<td align='center'>";
                Message = Message + "<b>Date</b>";
                Message = Message + "</td>";

                Message = Message + "<td align='center'>";
                Message = Message + "<b>Time</b>";
                Message = Message + "</td>";
        ...
        protected void Send_Click(object sender, EventArgs e)
        {
            ...
            if (RbtnYou.Checked || RbtnOther.Checked)
            {
                ...
                msg.Append("<b>The below table has the details for User Location.</b>");
                msg.AppendLine(); msg.AppendLine();
        ...
        private void MailsendYou(string msgBody)
        {
            try
            {
                ...
                send.SendEmail(EmailId, "User Location Report at " + GetCurrentDateTimeByUserId(), msgBody, ClientId);
        ...
        private void MailsendOther(string msgBody)
        {
            ...
            if (txtMailTo.Text != "" && Regex.IsMatch(txtMailTo.Text.Trim(), emailpattern))
            {
                try
                {
                    ...
                    dt = new DataTable();
                    send = new SendMailBAL();
                    send.SendEmail(txtMailTo.Text, "User Location Report at " + GetCurrentDateTimeByUserId(), msgBody, ClientId);
                    lblmsg.Text = "Mail sent successfully";
                    lblmsg.ForeColor = System.Drawing.Color.Green;
                }
                catch (Exception)
