...
namespace MobiOcean.MDM.BAL.BAL
{
    public class PaymentNewBAL
    {
        ...
        internal string categoryIdList;

        public PaymentNewBAL()
        {
            //
        ...
        public int insertPayOrder()
        {
            try
            {
                ...
                int res = Convert.ToInt32(payorder.InsertPaymentOrder(UserID, ClientId, RoleId, EmailId, OrderNo, categoryDuration, txtDuration, Convert.ToInt16(categoryNoofLicense), SubTotal, statusMessage, total, conformationemail, SmsCost, stateId, city, address1, discountedAmount, cgst, sgst, igst,GSTNo));
                return res;
            }
            catch(Exception)
            {
        ...
        public int insertPayOrder1()
        {
            try
            {
                payorder = new tblPaymentOrderTableAdapter();
                int res = Convert.ToInt32(payorder.InsertPaymentSubscriptionWithGST(ClientId, UserID, EmailId, city, address1, pincode, OrderNo, ProductKey, categoryIdList, categoryDuration, categoryNoofLicense, CategoryTotalAmount,PricePerUnit, total, promocode, discountedAmount, SubTotal, IsTrail, statusMessage, conformationemail, stateId, cgst, sgst, igst, GSTNo));
                return res;
            }
            catch (Exception)
            {
        ...
        public DataTable insertPayOrderOffline()
        {
            ...
            {
                payorder = new tblPaymentOrderTableAdapter();
                dt = payorder.InsertAndGetPaymentOrderDetails(UserID, ClientId, RoleId, EmailId, OrderNo, categoryDuration, txtDuration, Convert.ToInt16(categoryNoofLicense), SubTotal, statusMessage, total, conformationemail, SmsCost, stateId, cgst, sgst, igst);
                return dt;
            }
            catch
            {
        ...
        public DataTable GetPaymentdtlandUpdate1()
        {
            dt = new DataTable();
            try
            {
                payuMoney = new tblPayementPayUMoneyTableAdapter();
                dt = payuMoney.updatepaymentorder1(Convert.ToInt32(order_id), order_status);
                return dt;
            }
            catch(Exception ex)
            {
